...
2. MongoDB
\
**Note:** Refer this [repository](https://github.com/HwaiTengTeoh/Yelp-MongoDB-HDFS-PySpark/tree/main/Data%20Storage)

### Yelp Data Access and Querying
1. Access from HDFS or MongoDB to PySpark
2. Perform Querying on Yelp Data
\
**Note:** Refer this [repository](https://github.com/HwaiTengTeoh/Yelp-MongoDB-HDFS-PySpark/tree/main/Data%20Access)

### Yelp Data Analytics (No Demonstration)
1. MLib
2. GraphX

- Since this case study focuses more on big data management, thus there is no demonstration of code for machine learning using MLib and GraphX. However, the technique is explored by reviewing the existing work shared by some data enthusiasts in Medium Blog and GitHub repository. The implementation of the idea for the retail sector will be briefly discussed. 
- Firstly, MLib can be used to perform sentiment analysis on Yelp's reviews for the retail business and build a model to classify Yelp reviews as positive/negative based on the textual content. Word clouds can be produced to represent the positive and negative phrases for quick visualization of the reviews' sentiment from two distinct classes. Text analysis of reviews, tips and customer comments can suggest improvements for the retail business based on customer opinions and reviews. Some processing, such as tokenization, stemming, removal of stop words, and other text cleaning steps, are required before ingesting the model for training. The retail business can be informed about its competitors and what customers like about those retail businesses. The strategic team can discuss to improve in order to get new customers and retain current customers.
-  In addition, a model to predict the closure of a retail business can be developed by taking the attributes of the business and other relevant factors, e.g., Number of reviews, Number of tips, check-ins, price, etc., available on Yelp Data. 
-  The exploration of Yelp data can be extended using GraphX, a library for performing graph-parallel operations. GraphX allows the operation of ETL, computational of the iterative graph, and analysis. For example, the interaction among users can be explored to gauge users' preferences and understand their activity. A computation is performed to show the impact of a user on the network and its neighbors.
